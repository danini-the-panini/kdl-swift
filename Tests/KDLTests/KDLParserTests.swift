...
struct KDLParserTests {
    ...
    @Test static func testNode() throws {
        ...
        try #expect(parser.parse("node { node2 }") == KDLDocument([KDLNode("node", children: [KDLNode("node2")])]))
        try #expect(parser.parse("node { node2; node3 }") == KDLDocument([KDLNode("node", children: [KDLNode("node2"), KDLNode("node3")])]))
    }
    ...
    @Test static func testString() throws {
        ...
        try #expect(parser.parse(#"node -flag"#) == KDLDocument([KDLNode("node", arguments: [.string("-flag")])]))
        try #expect(parser.parse(#"node --flagg"#) == KDLDocument([KDLNode("node", arguments: [.string("--flagg")])]))
    ...
    @Test static func testVersionDirective() throws {
        let parser = KDLParser()
        #expect(throws: Never.self) { try parser.parse("/- kdl-version 2\nnode foo") }

        #expect(throws: (any Error).self) { try parser.parse("/- kdl-version 1\nnode \"foo\"") }
    }
}
